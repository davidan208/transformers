...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        # print("Done visual", hidden_states.shape)

        return hidden_states

class PT_mtfk(Qwen2_5_VisionTransformerPretrainedModel):
    def __init__(self, config, num_sp):
        super().__init__(config)

        if isinstance(num_sp, list) and all(isinstance(p, str) for p in num_sp):
            if len(num_sp) != 4:
                raise "Not enough path"
            

            def _Qwen2RMSnorm(hidden_states: torch.Tensor, eps: float = 1e-6):
                with torch.no_grad():
            ...
            sp7  = torch.load(num_sp[0], map_location = "cpu")
            sp15 = torch.load(num_sp[1], map_location = "cpu")
            sp23 = torch.load(num_sp[2], map_location = "cpu")
            sp31 = torch.load(num_sp[3], map_location = "cpu")

            temp_shape = sp7.shape
            if any(tensor_.shape != temp_shape for tensor_ in [sp7, sp15, sp23, sp31]):
                raise ValueError("Soft prompt got different shape")
            
            sp7_norm  = _Qwen2RMSnorm(sp7)
            sp15_norm = _Qwen2RMSnorm(sp15)
            sp23_norm = _Qwen2RMSnorm(sp23)
            sp31_norm = _Qwen2RMSnorm(sp31)


            self.soft_prompts = nn.ParameterDict({
                "layer_7":  nn.Parameter(sp7_norm.unsqueeze(0)),
                "layer_15": nn.Parameter(sp15_norm.unsqueeze(0)),
                "layer_23": nn.Parameter(sp23_norm.unsqueeze(0)),
                "layer_31": nn.Parameter(sp31_norm.unsqueeze(0))
            })
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        # prompts_to_add = self.soft_prompts.squeeze(0).to(hidden_states.device, hidden_states.dtype)
